...
heights %>% ggplot(aes(x = height)) + 
  geom_histogram(binwidth = 1, boundary = 0) # Seperated by 1, also alligned to the left
...
```{r}
#Histogram of all heights - binwidth 1
heights %>% ggplot(aes(x = height)) + 
...
```{r}
# Splits densities by sex
heights %>% ggplot(aes(x = height, group = sex)) + 
  geom_density()
```
...
```{r}
# Splits densities by sex w/ color lines
...
```{r}
# Splits densities by sex w/ color fill
heights %>% ggplot(aes(x = height, fill = sex, alpha = 0.5)) + 
  geom_density()

# Splits histograms by sex w/ color
...
```{r}
# Boxplots
heights %>% ggplot(aes(y = height, color = sex)) + 
  geom_boxplot()
```
...
(
```{r}
# Displays mean and median overall using a table
heights %>% 
...
```{r}
# Displays mean and median for both groups using a table
heights %>% 
  group_by(sex) %>% 
  summarise(mean = mean(height), median = median(height))
...
```{R}
# Table shows maximum and minimum height for both groups
heights %>% 
  group_by(sex) %>% 
  summarize(max_height = max(height), min_height = min(height))
...
#  ggplot(aes(sex, height)) + geom_bar()
